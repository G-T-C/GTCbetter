layui.config({
    ...
    table.render({
        elem: '#list'
        , height: 'full-10'
        , url: '/admin/Apply/extractList' //数据接口
        , page: true //开启分页
        , cols: [[ //表头
    ...
    table.on('tool(coinExtract)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'checkpass'){
            layer.confirm(
                ...
                {icon:3, 'title':'币种充值确认'},
                function(index){
                    $.get('/admin/Apply/extractCheckPass/id/'+id, function(res){
                        layer.msg(res.msg);
                        if(res.code == 0){
                            setTimeout(function(){
            ...
            layer.prompt({
                ...
                $.post('/admin/Apply/extractCheckReject', {'id': id, 'reason': value}, function(res){
                    layer.msg(res.msg);
                    if(res.code == 0){
                        setTimeout(function(){
                            window.location.reload()
