layui.config({
	...
	$(window).one("resize",function(){
		$(".newsAdd_btn").click(function(){
			var index = layui.layer.open({
				title : "添加轮播图",
				type : 2,
				content : "/admin/Shop_banner/addBanner/",
        ...
        form.on("submit(addNews)", function (data) {
            ...
            $.ajax({
               url: '/admin/Shop_banner/addBanner',
               type: 'POST',
               data: formdata,
               processData: false,
               contentType: false,
    ...
    $(window).one("resize",function(){
        $(".news_edit").click(function(){
            ...
            var index = layui.layer.open({
                title : "编辑",
                type : 2,
                content : "/admin/Shop_banner/editBanner?id="+id,
                success : function(layero, index){
                    setTimeout(function(){
	...
	$("body").on("click",".news_del",function(){  //删除
		...
		layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
            $.post('/admin/Shop_banner/delBanner',{
	...
	form.on("submit(editNews)", function(){
            var formdom = $('#userform')[0]
                ,formdata = new FormData(formdom);
            $.ajax({
                url: '/admin/Shop_banner/editBanner',
                type: 'POST',
                data: formdata,
